    ...
    public class PrimMinSpanningTree
    {
        ...
        public PrimMinSpanningTree(int Capacity, ImageGraph graph)
        {
            ...
            while (PQ.IsEmpty() == false)
            {
                ...
                Marked[MinNode.Id] = true; // mark as out of heap

                foreach (var next in DistinctColors)
                {
                    if(next == MinNode.Id)
                    {
                        continue;
                    }
                    long nextWeight = CalculateEuclideanDistance(MinNode.Id, next);
                    if (Marked[next] == false && Distance[next] > nextWeight)
                    {
                        PQ.UpdateHeap(next, nextWeight);
                        Parent[next] = MinNode.Id;
